...
import urllib.request, json, base64, os, time, re, hashlib, sqlite3, sys
...
# ═══════ Docs引擎(替代Onyx) ═══════
import sys as _sys
DOCS_ENGINE_PATH = os.path.expanduser("~/lgox-ops/scripts")
if DOCS_ENGINE_PATH not in _sys.path:
    _sys.path.insert(0, DOCS_ENGINE_PATH)
...
def _lge_candidates():
    ...
    try:
        ...
        try:
            with open(os.path.expanduser("~/lgox-lge-search-debug.log"), "a") as f:
                f.write(f"[{time.time()}] _lge_candidates OK: {len(genes)} genes\n")
        except:
            pass
        return genes
        ...
        try:
            with open(os.path.expanduser("~/lgox-lge-search-debug.log"), "a") as f:
                import traceback
                f.write(f"[{time.time()}] _lge_candidates ERROR: {e}\n{traceback.format_exc()}\n")
        except:
            pass
...
def _lge_search(query):
    ...
    try:
        ...
        try:
            with open(os.path.expanduser("~/lgox-lge-search-debug.log"), "a") as f:
                f.write(f"[{time.time()}] _lge_search '{query}': {len(candidates)} candidates, terms={terms}\n")
        except:
            pass
        scored = []
        for g in candidates:
        ...
        for item in scored:
            ...
            g_meta = {}
            for g in candidates:
                if g.get("gene_id") == gid:
                    g_meta = g.get("metadata", {})
                    break
            if isinstance(g_meta, str):
                try: g_meta = json.loads(g_meta)
                except: g_meta = {}
            quality = g_meta.get("quality", {})
            if isinstance(quality, dict) and quality.get("total", 0) > 0:
                item["_quality_score"] = quality["total"]
                item["_quality_grade"] = quality.get("grade", "?")
            else:
                item["_quality_score"] = item.get("score", 0) * 100
                item["_quality_grade"] = "?"
        scored.sort(key=lambda x: (x.get("_quality_score", 0), x.get("score", 0)), reverse=True)
        try:
            with open(os.path.expanduser("~/lgox-lge-search-debug.log"), "a") as f:
                f.write(f"[{time.time()}] _lge_search '{query}': {len(scored)} matches\n")
        except:
            pass
        return scored[:5]
    except Exception as e:
        try:
            with open(os.path.expanduser("~/lgox-lge-search-debug.log"), "a") as f:
                import traceback
                f.write(f"[{time.time()}] _lge_search ERROR for '{query}': {e}\n{traceback.format_exc()}\n")
        except:
            pass
        return []
...
# ═══════ Docs引擎(替代Onyx·自建文档库) ═══════
def _docs_search(query):
    """自建文档库BM25搜索·天枢本地·零远程依赖"""
    try:
        # 导入依赖(2035可靠·Python标准库)
        import sqlite3 as _sqlite3, os as _os
        index_db = _os.path.expanduser("~/lgox-ops/data/docs-index.db")
        if not _os.path.exists(index_db):
            return []
        conn = _sqlite3.connect(f"file:{index_db}?mode=ro", uri=True)
        try:
            rows = conn.execute(
                "SELECT d.path, d.title, snippet(docs_fts, 1, '<mark>', '</mark>', '...', 48) as snip, d.tags "
            ...
            like_q = f"%{query}%"
            rows = conn.execute(
                "SELECT path, title, substr(content, 1, 200), tags "
                "FROM docs WHERE title LIKE ? OR content LIKE ? LIMIT 10",
                (like_q, like_q)
            ).fetchall()
        conn.close()
        return [{"title": r[1], "path": r[0], "snippet": r[2], "tags": r[3] or ""} for r in rows]
    except Exception:
        return []
...
def unified_query(query, timeout=8):
    ...
    for eng, items in results.items():
        for item in items:
            gid = item.get("gene_id") or item.get("semantic_id")
            if gid:
                evidence.append({"gene_id": gid, "source": item.get("source", eng), 
                                 "engine": eng})
...
class UnifiedQueryHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        elif "/federated-search" in self.path:
            # 兼容旧联邦桥调用·映射sources→四引擎
            length = int(self.headers.get("Content-Length", 0))
            body = json.loads(self.rfile.read(length)) if length else {}
            query = body.get("query", body.get("q", ""))
            sources = body.get("sources", ["lge","docs","graph","bm25"])
            # 映射旧source名: onyx→docs
            engines = []
            for s in sources:
                if s == "onyx": engines.append("docs")
                elif s in ["lge","docs","graph","bm25"]: engines.append(s)
            if not engines: engines = ["lge","docs","graph","bm25"]
            if not query:
                self._json(400, {"error": "query参数必填"})
                return
    ...
    def do_GET(self):
        ...
        if up.path == "/" or up.path == "/health":
            ...
            self._json(200, {
                ...
                "version": "2.0.0",
                "layer": "知识层(Know)",
                "engines": ["lge", "docs", "neo4j_graph", "bm25"],
                "cache": stats,
            ...
            self._json(200, result)
        elif "/federated-search" in up.path:
            q = parse_qs(up.query).get("q", [""])[0]
            if not q:
                self._json(400, {"error": "q参数必填"})
                return
