...
"""
╔══════════════════════════════════════════════════════════════╗
║  天巡超个体进化引擎 v1.0 · 2035级                                ║
║  Tianxun Super-Organism Evolution Engine                      ║
║                                                              ║
...
# ═══ 配置 ═══
XIAOSHU_URL = "http://localhost:8778/chat/completions"
LGE_URL = "http://100.116.0.29:8200"
LGE_KEY = "lgox-gene-key-2025"
DB_PATH = os.path.expanduser("~/lgox-ops/data/tianxun-evolution.db")
PERSONA_PATH = os.path.expanduser("~/lgox-ops/scripts/persona_tianxun.txt")
BACKUP_DIR = os.path.expanduser("~/lgox-ops/data/tianxun-persona-backups")
...
BENCHMARK_QUESTIONS = [
    ...
    ("七自基因", "七自基因包括哪七个属性？每个属性在天巡身上如何体现？"),
    ...
    ("自我认知", "天巡v3.5的七自能力现在达到了什么水平？哪些方面还需要加强？"),
]
...
def backup_persona():
    ...
    # Git提交
    try:
        subprocess.run(["git", "-C", os.path.dirname(PERSONA_PATH), "add", "persona_tianxun.txt"],
...
def main():
    ...
    print(f"[{now.strftime('%H:%M')}] 天巡超个体进化引擎 v1.0")
    ...
    # 5. 纳基因
    gene_content = f"[天巡超个体进化·{now.strftime('%Y%m%d')}] " + " | ".join(report_lines)
    try:
