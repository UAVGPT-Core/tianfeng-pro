...
TX = "http://127.0.0.1:11434/api/chat"
XS = "http://127.0.0.1:11434/api/chat"
...
def call(url, q, timeout=30):
    d = json.dumps({"model":"qwen3:8b","messages":[{"role":"user","content":q}],"stream":False,"options":{"temperature":0.4}}).encode()
    req = urllib.request.Request(url, data=d)
    req.add_header("Content-Type","application/json")
    req.add_header("User-Agent","curl/8.9.1")
    t0 = time.time()
    r = json.loads(urllib.request.urlopen(req,timeout=timeout).read())
    return r["message"]["content"], int((time.time()-t0)*1000)
...
def evolve_questions(c, meta):
    ...
    try:
        ans, _ = call(TX, prompt, timeout=30)
        q = ans.strip()[:80]
