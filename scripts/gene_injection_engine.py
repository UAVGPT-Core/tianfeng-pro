...
BUILTIN_PATTERNS = {
    ...
    "graceful_shutdown": [
        "# PATTERN: 优雅关闭 (GENE-PRO-d6e7f8a9)",
        "import signal",
    ...
    "rpc_framework": [
        "# PATTERN: RPC框架核心模式 (GENE-PRO-RPC-v1)",
        "# 三层架构: 服务注册中心 → 请求序列化/反序列化 → RPC调用器",
        ...
        "                'params': self.params, 'id': self.request_id}",
        "",
        "class RPCResponse:",
        ...
        "# ─── RPC调用器 ───",
        "class RPCCaller:",
        "    \"\"\"调用远程服务: service.method(params) → RPCResponse\"\"\"",
        ...
        "            if method is None:",
        "                return RPCResponse(error=f'Method {request.method} not found',",
        ...
        "",
        "# 踩坑: (1)服务名/方法名大小写敏感 (2)参数必须匹配签名",
        "# (3)序列化JSON只支持基本类型 (4)Request ID用于追踪/去重/幂等",
    ],
}
