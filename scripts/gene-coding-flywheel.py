...
ALGORITHM_PATTERNS = {
    ...
    'majority_element_pattern': [
        "# PATTERN: Boyer-Moore多数投票算法 (GENE-SEM-MAJORITY-VOTE-v1)",
        "# 问题: 找到数组中出现次数 > n/2 的多数元素",
        "# 核心: 两阶段 — ①候选(candidate)阶段: 不同元素互相抵消; ②验证阶段: 确认候选出现次数>n/2",
        "def majority_element(nums):",
        "    \"\"\"",
        "    Boyer-Moore多数投票算法 O(n)时间 O(1)空间",
        "    原理: 投票&抵消。count为0时换候选人, 遇到相同+1, 不同-1",
        "    因为多数元素出现>n/2次, 抵消后一定剩下它",
        "    \"\"\"",
        "    candidate = None",
        "    count = 0",
        "    for num in nums:",
        ...
        "        else:",
        "            count -= 1",
        "    # 验证阶段(题目保证存在则直接返回candidate)",
        "    # nums.count(candidate) > len(nums) // 2 验证通过",
        "    return candidate",
        "",
        "# 变体1: 找出现次数 > n/3 的元素(最多2个)",
        "def majority_element_n3(nums):",
        "    \"\"\"Boyer-Moore变体: 找出现>n/3的元素·双候选人\"\"\"",
        "    c1 = c2 = None",
        "    cnt1 = cnt2 = 0",
        "    for num in nums:",
        "        if num == c1: cnt1 += 1",
        ...
        "    # 验证: 检查实际出现次数",
        "    n = len(nums)",
        "    return [x for x in (c1, c2) if x is not None and nums.count(x) > n // 3]",
        "",
        "# 踩坑:",
        "# (1) 第一轮只出候选人, 必须验证! BUG: 直接返回candidate而candidate可能无效",
        "# (2) count初始为0, 第一个数就是候选人",
        "# (3) 变体n/3时两个候选人必须不同(初始None处理)",
        "# (4) 此题前提: 多数元素一定存在。若不存在需验证阶段",
    ],
    ...
    'backtracking_pattern': [
        "# PATTERN: 回溯算法·全排列/子集/组合 (GENE-SEM-BACKTRACKING-v1)",
        "# 回溯算法框架: 路径·选择列表·终止条件",
        ...
        "",
        "# ⚠️ 踩坑:",
        "# (1) 结果必须path[:]复制! 用path本体会被后续pop清空",
        "# (2) 全排列用used数组标已选, 子集/组合用start控制不回头",
        "# (3) 剪枝: 全排列排序+nums[i]==nums[i-1] and not used[i-1]跳过同层重复",
        "# (4) 时间复杂度: 全排列O(n!), 子集O(2^n), 组合O(C(n,k))",
        "# (5) 空间: 递归栈深度O(n), path长度O(n)",
    ],
    'singleton_pattern': [
...
FALLBACK_PATTERNS = {
    ...
    "sql_safe": [
        "# PATTERN: SQL注入防护·参数化查询 (GENE-PRO-SQL-SAFE-v1)",
        "# 黄金法则: 永远不要用f-string/format/%拼接SQL参数",
        "# ❌ 危险: cursor.execute(f\"SELECT * FROM users WHERE id = {user_id}\")",
        "# ✅ 安全: cursor.execute(\"SELECT * FROM users WHERE id = ?\", (user_id,))",
        "",
        "# ── SQLite参数化 ──",
        "# ?占位符: cursor.execute(\"INSERT INTO t VALUES (?, ?)\", (v1, v2))",
        "# 命名占位符: cursor.execute(\"INSERT INTO t VALUES (:a, :b)\", {\"a\": v1, \"b\": v2})",
        "",
        "# ── MySQL/psycopg2参数化 ──",
        "# %s占位符: cursor.execute(\"INSERT INTO t VALUES (%s, %s)\", (v1, v2))",
        "# 注意: 表名/列名不能用参数化, 必须白名单校验",
        "",
        "# ── SQLAlchemy ORM安全 ──",
        "# session.query(User).filter(User.id == user_id).first()  # 安全",
        "# text(\"SELECT * FROM users WHERE id = :uid\").params(uid=user_id)  # 安全",
        "",
        "# ── 动态表名列名白名单 ──",
        "ALLOWED_TABLES = {'users', 'orders', 'products'}",
        "if table_name not in ALLOWED_TABLES:",
        "    raise ValueError(f'禁止的表名: {table_name}')",
        "# 只有通过白名单校验后才拼接, 且用f-string只拼标识符不拼数据",
        "sql = f\"SELECT * FROM {table_name} WHERE id = ?\"",
        "cursor.execute(sql, (record_id,))",
        "",
        "# 踩坑: 存储过程也可能有注入(EXEC/EXECUTE拼接)",
        "# 踩坑: ORDER BY/ASC/DESC也不能参数化, 用白名单",
        "# 踩坑: LIKE '%' + ? + '%' 对SQLite安全, 但对SQL Server需要特殊处理",
    ],
}
# ── 数据库内核模式（Join算子 · 2026-07-17 补充覆盖数据库连接缺口）──
DATABASE_JOIN_PATTERNS = {
    "join_operators_pattern": [
        "# PATTERN: 数据库连接算子·Nested Loop / Hash / Merge Join (GENE-DB-JOIN-v1)",
        "# ── 测试数据 ──",
        "# employees = [(1,'Alice',101), (2,'Bob',102), (3,'Charlie',101)]",
        "# depts = [(101,'Engineering'), (102,'Marketing')]",
        "# 外键: employee[2] == dept[0]",
        "",
        "# ── 1) Nested Loop Join (O(n*m) · 小表驱动大表) ──",
        "def nested_loop_join(outer, inner, outer_key_fn, inner_key_fn):",
        "    \"\"\"嵌套循环连接: 外层每行遍历内层全表\"\"\"",
        "    result = []",
        "    for o_row in outer:",
        "        o_key = outer_key_fn(o_row)",
        "        for i_row in inner:",
        "            if inner_key_fn(i_row) == o_key:",
        "                result.append(o_row + i_row)",
        "    return result",
        "",
        "# 优化: 小表在外层(减少内层遍历次数)",
        "# 适用: 内表很小(<100行) 或 不需要索引",
        "# 踩坑: outer放小表! O(outer_rows * inner_rows)",
        "",
        "# ── 2) Hash Join (O(n+m) · 等值连接专用) ──",
        "def hash_join(outer, inner, outer_key_fn, inner_key_fn):",
        "    \"\"\"哈希连接: 构建哈希表→探测匹配\"\"\"",
        "    # Phase 1: Build — 用较小表构建哈希表",
        "    hashtable = {}",
        "    for i_row in inner:",
        "        key = inner_key_fn(i_row)",
        "        hashtable.setdefault(key, []).append(i_row)",
        "",
        "    # Phase 2: Probe — 遍历大表探测",
        "    result = []",
        "    for o_row in outer:",
        "        key = outer_key_fn(o_row)",
        "        matches = hashtable.get(key, [])",
        "        for i_row in matches:",
        "            result.append(o_row + i_row)",
        "    return result",
        "",
        "# 优化: 用小表建哈希表(减少内存+加速build)",
        "# 适用: 大表等值连接, 无索引, 大数据量(O(n+m)优于NL的O(n*m))",
        "# 踩坑: (a) 非等值条件不可用 (b) 大key浪费内存 (c) hash冲突退化为链表",
        "",
        "# ── 3) Merge Join (O(n+m) · 需排序输入) ──",
        "def merge_join(left, right, left_key_fn, right_key_fn):",
        "    \"\"\"归并连接: 双指针同步扫描已排序的两表\"\"\"",
        "    result = []",
        "    i = j = 0",
        "    while i < len(left) and j < len(right):",
        "        l_key = left_key_fn(left[i])",
        "        r_key = right_key_fn(right[j])",
        "",
        "        if l_key < r_key:",
        "            i += 1  # 左表太小, 前进左指针",
        "        elif l_key > r_key:",
        "            j += 1  # 右表太小, 前进右指针",
        "        else:",
        "            # 等值匹配: 收集所有同key行(可能有重复)",
        "            i2, j2 = i, j",
        "            while i2 < len(left) and left_key_fn(left[i2]) == l_key:",
        "                j2 = j",
        "                while j2 < len(right) and right_key_fn(right[j2]) == r_key:",
        "                    result.append(left[i2] + right[j2])",
        "                    j2 += 1",
        "                i2 += 1",
        "            i, j = i2, j2",
        "    return result",
        "",
        "# 优化: 输入必须按join key排序(否则需要Sort-Merge)",
        "# 适用: (a) 输入已排序(索引扫描) (b) 非等值连接(<, <=, >, >=) (c) 大数据量",
        "# 踩坑: (a) 未排序输入需先Sort→O(nlogn+mlogm) (b) 一对多场景需要嵌套处理重复值",
        "",
        "# ── 4) 查询优化器: 自动选择Join算法 ──",
        "class JoinOptimizer:",
        "    \"\"\"根据统计信息自动选择最优Join策略\"\"\"",
        "    @staticmethod",
        "    def choose_strategy(outer_rows, inner_rows, join_type='eq', outer_sorted=False, inner_sorted=False):",
        "        \"\"\"选择最优Join算法\"\"\"",
        "        if join_type != 'eq':",
        "            # 非等值连接: Merge Join(需排序)或Nested Loop",
        "            return 'merge_join' if (outer_sorted and inner_sorted) else 'nested_loop_join'",
        "        # 等值连接: 优先Hash Join(无排序要求)",
        "        if min(outer_rows, inner_rows) < 100:",
        "            return 'nested_loop_join'  # 小表用NL更简单",
        "        if outer_sorted and inner_sorted:",
        "            return 'merge_join'  # 已排序用Merge无额外开销",
        "        return 'hash_join'  # 大表无排序用Hash",
        "",
        "# ── 使用示例 ──",
        "# if __name__ == '__main__':",
        "#     emps = [(1,'Alice',101), (2,'Bob',102), (3,'Charlie',101)]",
        "#     depts = [(101,'Engineering'), (102,'Marketing')]",
        "#     get_dept_id = lambda e: e[2]   # employee[2] = dept_id",
        "#     get_dept_key = lambda d: d[0]  # dept[0] = dept_id",
        "#     print('NLJ:', nested_loop_join(emps, depts, get_dept_id, get_dept_key))",
        "#     print('HJ:', hash_join(emps, depts, get_dept_id, get_dept_key))",
        "#     emps_sorted = sorted(emps, key=get_dept_id)",
        "#     depts_sorted = sorted(depts, key=get_dept_key)",
        "#     print('MJ:', merge_join(emps_sorted, depts_sorted, get_dept_id, get_dept_key))",
        "#     print('Opt:', JoinOptimizer.choose_strategy(100, 10000, 'eq', False, False))  # → hash_join",
        "",
        "# ⚠️ 踩坑总结:",
        "# (1) Nested Loop: 一定要小表在外! 10*10K=100K vs 10K*10=100K(一样), 但10*1M=10M vs 1M*10=10M(一样)",
        "#     实际差异: 内层重复加载缓存/索引, 小表在外层减少IO",
        "# (2) Hash Join: 建表表选择小表! build_time + probe_time, 大表建表浪费内存和CPU",
        "# (3) Merge Join: 同key重复需要嵌套循环收集所有匹配, 最坏退化为O(n*m)",
        "# (4) 实际数据库: 还会考虑 可用索引/内存预算/并行度/数据倾斜",
    ],
}
BUILTIN_PATTERNS.update(DATABASE_JOIN_PATTERNS)

# ── 网络协议模式（WebSocket + HTTP/2 · 2026-07-17 补充覆盖网络编程缺口）──
NETWORK_PROTOCOL_PATTERNS = {
    # ── WebSocket · RFC 6455 ──
    "websocket_handshake": [
        "# PATTERN: WebSocket 升级握手·RFC 6455 (GENE-PRO-WS-HANDSHAKE-v1)",
        "import hashlib, base64",
        "MAGIC_GUID = '258EAFA5-E914-47DA-95CA-C5AB0DC85B11'",
        "",
        "def compute_accept_key(key: str) -> str:",
        "    '''计算服务端 Sec-WebSocket-Accept'''",
        "    return base64.b64encode(hashlib.sha1((key + MAGIC_GUID).encode()).digest()).decode()",
        "",
        "def build_handshake_response(key: str) -> bytes:",
        ...
        "    ).encode()",
        "",
        "# 踩坑: Magic GUID 是固定常量, 非随机字符串",
        "# 踩坑: Connection 头必须包含 'Upgrade' (逗号分隔列表)",
        "# 踩坑: Sec-WebSocket-Key 是16字节base64, 服务端不验证内容但必须计算accept",
    ],
    "websocket_frame": [
        "# PATTERN: WebSocket 帧解析·RFC 6455 §5.2 (GENE-PRO-WS-FRAME-v1)",
        "import struct",
        "OPCODE = {0: 'CONT', 1: 'TEXT', 2: 'BINARY', 8: 'CLOSE', 9: 'PING', 10: 'PONG'}",
        "",
        ...
        "    mask_key = data[offset:offset+4] if masked else None; offset += 4 if masked else 0",
        "    payload = data[offset:offset+length]",
        "    if mask_key:",
        "        payload = bytes(b ^ mask_key[i % 4] for i, b in enumerate(payload))",
        "    return {'fin': fin, 'opcode': OPCODE.get(opcode, f'UNK({opcode})'), 'payload': payload}",
        "",
        "# 踩坑: 控制帧(Close/Ping/Pong)不可分片, 负载<125B",
        "# 踩坑: 客户端帧必须mask, 服务端帧禁止mask",
    ],
    # ── HTTP/2 多路复用 · RFC 7540/9113 ──
    "http2_frame": [
        "# PATTERN: HTTP/2 帧格式·RFC 7540 §4.1 (GENE-PRO-HTTP2-FRAME-v1)",
        "# HTTP/2 帧通用头: Length(3) | Type(1) | Flags(1) | StreamID(4) = 9字节",
        "import struct",
        ...
        "",
        "def parse_http2_frame(data: bytes) -> dict:",
        "    '''解析HTTP/2帧头'''",
        "    if len(data) < 9:",
        "        return {'error': 'frame too short'}",
        ...
        "    return {",
        "        'length': length, 'type': FRAME_TYPES.get(frame_type, f'UNK({frame_type})'),",
        "        'flags': flags, 'stream_id': stream_id, 'payload': payload,",
        "    }",
        "",
        "# 踩坑: StreamID最高位保留(0x80000000), 实际31位",
        "# 踩坑: Length是3字节无符号(最大16383), 9字节frame header不计入",
    ],
    "http2_multiplexing": [
        "# PATTERN: HTTP/2 Stream多路复用·RFC 7540 §5 (GENE-PRO-HTTP2-MUX-v1)",
        "# 核心: 单个TCP连接承载多个并行的HTTP请求/响应流",
        "# Stream状态机: idle→open→half-closed(local/remote)→closed",
        "# 帧交错: 不同Stream的帧在连接上交错传输, 无队头阻塞(相对HTTP/1.1 pipelining)",
        "",
        "STREAM_STATES = {",
        "    'idle': '初始状态, 收到或发送HEADERS/PRIORITY后离开',",
        "    'reserved_local': 'PUSH_PROMISE已发送, 等待推送',",
        "    'reserved_remote': 'PUSH_PROMISE已接收',",
        "    'open': '双方均可发送帧',",
        "    'half_closed_local': '本端发送END_STREAM, 仍可接收',",
        "    'half_closed_remote': '对端发送END_STREAM, 本端仍可发送',",
        "    'closed': '终结, 或RST_STREAM强制关闭',",
        "}",
        "",
        ...
        "    def __init__(self):",
        "        self.streams = {}",
        "        self.next_stream_id = 1  # 客户端流ID:奇数; 服务端:偶数",
        "        self.flow_window = 65535  # 初始流控窗口",
        "    ",
        "    def create_stream(self):",
        "        sid = self.next_stream_id; self.next_stream_id += 2",
        "        self.streams[sid] = {'state': 'idle', 'buffer': [], 'window': self.flow_window}",
        "        return sid",
        "    ",
        "    def send_frame(self, stream_id, frame_type, payload, flags=0):",
        "        '''发送帧到指定Stream'''",
        "        frame_header = struct.pack('!IBBI', len(payload), frame_type, flags, stream_id)",
        "        return frame_header + payload",
        "",
        "# 踩坑: 客户端流ID为奇数(1,3,5...), 服务端为偶数(2,4,6...)",
        "# 踩坑: HEADERS帧后紧接CONTINUATION(非END_HEADERS时)",
        "# 踩坑: 对流控敏感, WINDOW_UPDATE帧定期发送防止死锁",
    ],
    "hpack_compression": [
        "# PATTERN: HPACK头部压缩·RFC 7541 (GENE-PRO-HPACK-v1)",
        "# HPACK: 用静态表(61项)+动态表+霍夫曼编码压缩HTTP头部",
        "# 静态表: 常见头(name+value)如 :method: GET (索引2), :status: 200 (索引8)",
        "# 编码类型: Indexed(1), Literal with Incremental Indexing(01), Literal without Indexing(0000)",
        "",
        "HPACK_STATIC_TABLE = [",
        ...
        "][:61]  # 仅前61项, 完整见RFC 7541附录A",
        "",
        "# 霍夫曼编码后的:method: GET 仅为4字节 (原始~13字节)",
        "# 踩坑: 动态表大小受SETTINGS_HEADER_TABLE_SIZE限制(默认4096)",
        "# 踩坑: 霍夫曼编码表固定(RFC 7541附录B), 不可协商",
        "# 踩坑: 动态表采用FIFO+LRU, 超限时从尾部移除",
    ],
    "http2_connection_preface": [
        "# PATTERN: HTTP/2 连接前导+SETTINGS (GENE-PRO-HTTP2-PREFACE-v1)",
        "# 客户端发送24字节前导: PRI * HTTP/2.0\\r\\n\\r\\nSM\\r\\n\\r\\n",
        "# 然后立即SETTINGS帧(空帧也可, 用于启用流控等)",
        "",
        "HTTP2_CLIENT_PREFACE = b'PRI * HTTP/2.0\\r\\n\\r\\nSM\\r\\n\\r\\n'",
        "",
        "def build_settings_frame(settings: dict = None) -> bytes:",
        "    '''构建HTTP/2 SETTINGS帧'''",
        "    if settings is None:",
        "        settings = {1: 4096, 2: 0, 3: 65535, 4: 65535}  # header_table_size=4096, enable_push=0, init_window=65535, max_frame=65535",
        "    payload = b''",
        "    for key, value in settings.items():",
        "        payload += struct.pack('!HI', key, value)",
        "    # SETTINGS帧: type=4, flags=0, stream_id=0",
        "    return struct.pack('!IBBI', len(payload), 4, 0, 0) + payload",
        "",
        "# 踩坑: SETTINGS帧必须stream_id=0, 否则是连接错误(PROTOCOL_ERROR)",
        "# 踩坑: SETTINGS_ACK必须在收到SETTINGS后立即回复(flags=0x01)",
        "# 踩坑: INITIAL_WINDOW_SIZE改变影响所有已有Stream的发送窗口",
    ],
}
...
BUILTIN_PATTERNS.update(NETWORK_PROTOCOL_PATTERNS)  # 网络协议模式覆盖
...
def search_builtin_patterns(task_description):
    ...
    keyword_map = {
        ...
        "多数元素|majority element|majority_element|Boyer|Boyer-Moore|摩尔投票|摩尔|投票算法|众数|出现次数.*一半|> n/2|> n/3": "majority_element_pattern",
        ...
        "回溯|backtracking|全排列|排列|组合|组合数|子集|permutations|permute|subsets|combinations|combine|dfs.*回溯|路径.*选择|撤销|剪枝|used数组|visited|状态重置|深度优先.*搜索.*树|递归.*搜索": "backtracking_pattern",
        ...
        "websocket|ws|握手|upgrade|101 switching|handshake|协议升级|ws握手|websocket连接": "websocket_handshake",
        "帧|frame|opcode|payload|mask|unmask|数据帧|websocket帧|帧解析|帧格式|data frame|control frame": "websocket_frame",
        "http upgrade|协议切换|upgrade header|connection upgrade|协议协商|upgrade|协议升级": "websocket_handshake",
        "心跳|ping|pong|保活|keepalive|heartbeat|重连|reconnect|断线|断连|连接保持|长连接": "http2_connection_preface",
        "http2|http/2|h2|http2帧|http2协议|rfc7540|rfc9113|http2帧格式|帧头|frame header|type|flags|stream": "http2_frame",
        "多路复用|multiplex|stream multiplexing|stream交错|并发请求|并行流|stream id|stream状态|流状态|connection multiplexing": "http2_multiplexing",
        "hpack|头部压缩|header compression|静态表|动态表|霍夫曼|huffman|header table|压缩头部|伪头|伪头部|:method|:path|:scheme|:authority|:status": "hpack_compression",
